...
const instagramLogger = createLogger('InstagramDownloader');
...
export class InstagramDownloader {
  ...
  async verifyLogin(): Promise<boolean> {
    try {
      ...
      if (response?.user?.username) {
        instagramLogger.info('Successfully logged in as:', response.user.username);
        return true;
      }
      ...
      if (this.isInstagramError(error)) {
        instagramLogger.error('Login verification failed:', error.message);
        if (error.response?.data?.message) {
          instagramLogger.error('Instagram message:', error.response.data.message);
        }
      } else {
        instagramLogger.error('Unknown login verification error:', error);
      }
      return false;
    }
  }
  ...
  async downloadProfileImages(username: string): Promise<string[]> {
    try {
      ...
      instagramLogger.debug(`Fetching profile for: ${username}`);
      await this.waitForRateLimit();

      try {
        // Get user info first
        const userInfo = await this.ig.fetchUser(username);
        instagramLogger.info(`Found profile: ${userInfo.username}`);
        instagramLogger.info(`Total posts: ${userInfo.post_count}`);
        instagramLogger.info(`Will download up to ${this.maxPosts} most recent posts`);
        ...
        while (hasNextPage && totalDownloaded < this.maxPosts) {
          ...
          if (!edges || !Array.isArray(edges)) {
            instagramLogger.debug('No posts found in response:', response.data);
            break;
          }

          instagramLogger.debug(`Found ${edges.length} posts in current page`);
          ...
          for (const edge of edges) {
            ...
            instagramLogger.debug(`Downloading: ${filename}`);
            ...
            try {
              ...
              totalDownloaded++;
              instagramLogger.info(
                `Successfully downloaded: ${filename} (${totalDownloaded}/${this.maxPosts})`
              );
              await this.sleep(1000);
            } catch (error) {
              instagramLogger.error(`Failed to download ${filename}:`, error);
              continue;
            }
          }

          if (totalDownloaded >= this.maxPosts) {
            instagramLogger.info(`Reached maximum posts limit (${this.maxPosts})`);
            break;
          }
          ...
          if (hasNextPage) {
            instagramLogger.debug('Waiting before fetching next page...');
            await this.sleep(2000);
          }
        }

        if (posts.length > 0) {
          instagramLogger.info('Saving metadata to collection...');
          await this.saveMetadata(posts);
        }

        instagramLogger.info(`\nDownload summary:`);
        instagramLogger.info(`Total files downloaded: ${downloadedFiles.length}`);
      ...
      if (this.isInstagramError(error)) {
        instagramLogger.error('Error downloading Instagram images:', error.message);
        throw new Error(error.message);
      }
      instagramLogger.error('Unknown error downloading Instagram images:', error);

...
const hooks = [
  ...
  },
  {
    ...
    name: 'runLineCount',
    description:
      'Estimate the count amount of javascript lines for this project. (Does not include JS in Svelte files)',
    run: ({ settings }) => {
  ...
  },
  {
    ...
    run: async ({ settings, data }) => {
      const externalData = await fetch('https://api.github.com/repos/mldangelo/personal-site/releases').then((res) =>
        res.json(),
      );
